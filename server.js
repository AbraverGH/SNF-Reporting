...
/* ------------------------------------------------------------------
   ...
   served from the same backend. Two import slots: the Length of Stay
   report, and the Payer Mapping (built from the NCS Payer Listing's
   "List of Payors Detail" + "Sheet1" tabs) — the mapping ships with a
   built-in default (see public/index.html) but can be refreshed here
   whenever payor rates/contracts change, without a code redeploy.
------------------------------------------------------------------ */
...
function parsePayerMapWorkbookBuffer(buffer) {
  ...
  if (wb.SheetNames.includes('List of Payors Detail')) {
    ...
    for (let r = headerIdx + 1; r < grid.length; r++) {
      ...
      if (seen[key]) continue; // de-dupe — this sheet has one row per rate code, many per payor
      seen[key] = true;
