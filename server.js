...
app.post('/api/upload-pcc/:sourceKey', upload.single('file'), (req, res) => {
  try {
    ...
    if (sourceKey === 'pccJournal') {
      data = parsePCCJournalCSVBuffer(req.file.buffer);
      ...
      data = parsePCCAgingReportCSVBuffer(req.file.buffer);
...
app.post('/api/upload-pcc-medicare/:sourceKey', upload.single('file'), (req, res) => {
  try {
    ...
    const data = parsePCCMedicareSalesCSVBuffer(req.file.buffer);
